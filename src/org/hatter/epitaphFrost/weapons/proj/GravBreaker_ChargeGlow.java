...
public class GravBreaker_ChargeGlow extends CombatEntityPluginWithParticles {
    ...
    public static float ARC_RATE_MULT = 4f;

    //public static int MAX_ARC_RANGE = 300;
    public static int MAX_ARC_RANGE = 150;
    //public static int ARCS_ON_HIT = 15;

    public static float SHIELD_DAMAGE_MULT = 1.3f;
    public static float SHIELD_DAMAGE_DEBUFF_DUR = 6f;

    //public static Color UNDERCOLOR = RiftCascadeEffect.EXPLOSION_UNDERCOLOR;
    public static Color UNDERCOLOR = new Color(0,255,255,100);
    //public static Color RIFT_COLOR = RiftCascadeEffect.STANDARD_RIFT_COLOR;
    public static Color RIFT_COLOR = new Color(50,250,255,100);
    public static Color MISC_COLOR = new Color(255,255,255,100);


    public static Object STATUS_KEY = new Object();


    public static class GBShieldDebuff implements AdvanceableListener {
        public static String DEBUFF_ID = "grav_breaker_shield_debuff";

        public ShipAPI ship;
        public float dur = SHIELD_DAMAGE_DEBUFF_DUR;
        public GBShieldDebuff(ShipAPI ship) {
            this.ship = ship;

            ship.getMutableStats().getShieldDamageTakenMult().modifyMult(DEBUFF_ID,  SHIELD_DAMAGE_MULT);
            ship.getMutableStats().getShieldUpkeepMult().modifyMult(DEBUFF_ID,  SHIELD_DAMAGE_MULT);
            //ship.getMutableStats().getCombatWeaponRepairTimeMult().modifyMult(DEBUFF_ID, 1f/ SHIELD_DAMAGE_MULT);
        }

        public void resetDur() {
            dur = SHIELD_DAMAGE_DEBUFF_DUR;
        }
        ...
        public void advance(float amount) {
            ...
            if (Global.getCurrentState() == GameState.COMBAT &&
                ...
                Global.getCombatEngine().maintainStatusForPlayerShip(STATUS_KEY,
                        ...
                        "GRAVITON BREAKER", "SHIELD DAMAGE / UPKEEP INCREASED: " + (int)Math.max(1, Math.round(dur)) + " SEC", true);
            }
    ...
    public GravBreaker_ChargeGlow(WeaponAPI weapon) {
        super();
        this.weapon = weapon;
    ...
    public void spawnArc() {
        ...
        if (target != null) {
            ...
            if (target instanceof ShipAPI) {
                ...
                List<GBShieldDebuff> listeners = s.getListeners(GBShieldDebuff.class);
                if (listeners.isEmpty()) {
                    s.addListener(new GBShieldDebuff(s));
                } else {
...
//		color = Misc.scaleAlpha(color, b);
        ...
        if (proj != null) {
            ...
            if (proj.getElapsed() > 0.2f) {
                //addDarkParticle(size, in, out, 0.5f, size * 0.5f * velMult, 0f, color);
                addParticle(size, in, out, 0.5f, size * 0.5f * velMult, 0f, color.WHITE);
            ...
            if (proj.getElapsed() > 0.4f) {
                //addDarkParticle(size * 1f, in, out, 0.3f, size * 0.5f * velMult, 0f, color);
                addParticle(size * 1f, in, out, 0.3f, size * 0.5f * velMult, 0f, color.WHITE);
                Vector2f offset = new Vector2f(dir);
                offset.scale(size * 1.2f + (float) Math.random() * 0.2f);
                Vector2f.add(prev.offset, offset, prev.offset);
            }
            if (proj.getElapsed() > 0.6f) {
                //addDarkParticle(size * .8f, in, out, 0.1f, size * 0.5f * velMult, 0f, color);
                addParticle(size * .8f, in, out, 0.1f, size * 0.5f * velMult, 0f, color.WHITE);
                Vector2f offset = new Vector2f(dir);
...
//		Vector2f.add(prev.offset, dir, prev.offset);
    ...
    public void spawnEMPParticles(EMPArcHitType type, Vector2f point, CombatEntityAPI target) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            float s = size * 1.5f * (0.5f + (float) Math.random() * 0.5f);

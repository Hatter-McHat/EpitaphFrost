...
//copied from RAT
class VestigeThemeGenerator : BaseThemeGenerator() {
    ...
    override fun getThemeId(): String {
        return EFTags.VESTIGE_THEME_ID
    }
    ...
    {
        data.system.addTag(EFTags.VESTIGE_GATE)
        data.system.addTag(EFTags.THEME_VESTIGE)
    ...
    fun populateNonMain(data: StarSystemData)
    {
        ...
        data.system.addTag(EFTags.THEME_VESTIGE)
        data.system.addTag(EFTags.THEME_VESTIGE_SECONDARY)
    ...
    fun generateMirrorCopy(ogData: StarSystemData) : StarSystemAPI
    {
        ...
        mirroredSystem.backgroundTextureFilename = "graphics/EpitaphFrost/backgrounds/vestige_bg.jpg"
        mirroredSystem.addTag(EFTags.THEME_VESTIGE_COPY)
       ...
       /* if (mirroredSystem is BaseLocation)
        ...
        var mirroredStar = mirroredSystem.initStar("${ogStar.id}-vestige","ef_dead_star", ogStar.radius, 200f)
        mirroredSystem.lightColor = Color(0,200,100,150)
        ...
        for (ogPlanet in ogSystem.planets)
        {
            ...
            var type = "ef_vestige_planet"

            var mirroredPlanet = mirroredSystem.addPlanet("${ogPlanet.id}-vestige", mirroredStar, ogPlanet.name,
        ...
        for (ogMoon in ogSystem.planets)
        {
            ...
            var mirroredMoon = mirroredSystem.addPlanet("${ogMoon.id}-vestige", ogFocus.mirroredEntity, ogMoon.name,
                "ef_vestige_planet", ogMoon.circularOrbitAngle, ogMoon.radius, ogMoon.circularOrbitRadius, ogMoon.circularOrbitPeriod)
 ...
 */
    ...
    fun generateGate(data: StarSystemData, focus: MirrorEntity, orbitRadius: Float) : MirrorEntity
    {
        ...
        var ogTear = data.system.addCustomEntity("${data.system.id}_vestige_tear", "Anomalous Gate", "ef_vestige_gate", Factions.NEUTRAL)
        ...
        ogTear.addTag(EFTags.VESTIGE_GATE)
        var mirrorTear = focus.mirroredEntity.starSystem.addCustomEntity("${focus.mirroredEntity.starSystem.id}_vestige_tear", "Anomalous Gate", "ef_vestige_gate", Factions.NEUTRAL)
        ...
        mirrorTear.addTag(EFTags.VESTIGE_GATE)
        /*
        ...
        */
        ...
        if (ogPlugin is VestigeGate)
        {
            ogPlugin.teleportLocation = mirrorTear
        }

        if (mirrorPlugin is VestigeGate)
        {
    ...
    {
        //faction + station
        val fleet = FleetFactoryV3.createEmptyFleet("ef_vestige", FleetTypes.BATTLESTATION, null)

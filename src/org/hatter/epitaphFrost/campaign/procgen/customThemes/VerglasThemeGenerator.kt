package  org.hatter.epitaphFrost.campaign.procgen.customThemes

import org.hatter.epitaphFrost.campaign.plugins.entities.VerglasGate
import org.hatter.epitaphFrost.campaign.procgen.EFProcgenUtility
import org.hatter.epitaphFrost.scripts.VestigeBaseFleetManager
...
//copied from RAT
class VerglasThemeGenerator : BaseThemeGenerator() {
    ...
    override fun getThemeId(): String {
        return EFTags.VERGLAS_THEME_ID
    }
    ...
    override fun generateForSector(context: ThemeGenContext?, allowedSectorFraction: Float) {
        ...
        //gets available constellations that haven't been used yet
        ...
        for (constellation in constellations!!)
        {
            ...
            val mainCandidates = EFProcgenUtility.getScoredSystemsByFilter(systems) {
                it.system.hasTag(Tags.THEME_MISC)
                //
                && !it.system.hasTag(EFTags.THEME_VERGLAS)
                && !it.system.hasTag(EFTags.THEME_VESTIGE)
                //
    ...
    {
        data.system.addTag(EFTags.VERGLAS_GATE)
        data.system.addTag(EFTags.THEME_VERGLAS)
    ...
    {
        data.system.addTag(EFTags.THEME_VERGLAS)
        data.system.addTag(EFTags.THEME_VERGLAS_SECONDARY)
    ...
    fun generateMirrorCopy(ogData: StarSystemData) : StarSystemAPI
    {
        ...
        mirroredSystem.backgroundTextureFilename = "graphics/EpitaphFrost/backgrounds/verglas_bg.jpg"
        mirroredSystem.addTag(EFTags.THEME_VERGLAS_COPY)
       ...
       /* if (mirroredSystem is BaseLocation)
        ...
        var mirroredStar = mirroredSystem.initStar("${ogStar.id}-verglas","ef_icy_star", ogStar.radius, 200f)
        mirroredSystem.lightColor = Color(0,255,255,150)
        ...
        for (ogPlanet in ogSystem.planets)
        {
            ...
            var type = "ef_verglas_planet"

            var mirroredPlanet = mirroredSystem.addPlanet("${ogPlanet.id}-verglas", mirroredStar, ogPlanet.name,
        ...
        for (ogMoon in ogSystem.planets)
        {
            ...
            var mirroredMoon = mirroredSystem.addPlanet("${ogMoon.id}-verglas", ogFocus.mirroredEntity, ogMoon.name,
                "ef_verglas_planet", ogMoon.circularOrbitAngle, ogMoon.radius, ogMoon.circularOrbitRadius, ogMoon.circularOrbitPeriod)
        ...
        ogSystem.memoryWithoutUpdate.set("\$ef_mirrored_entities", mirrorEntities)
...
/*
        ...
        if (locations != null && !locations.isEmpty)
        {
            ...
            triResearchStation.entity.addTag(EFTags.TAG_CHIRAL_STATION1)
        }
...
/*
    fun generateHullmodStation(data: StarSystemData)
    {
        ...
        station.entity.addTag(EFTags.TAG_CHIRAL_STATION2)
 ...
 */
    ...
    fun generateGate(data: StarSystemData, focus: MirrorEntity, orbitRadius: Float) : MirrorEntity
    {
        ...
        var ogTear = data.system.addCustomEntity("${data.system.id}_verglas_tear", "Anomalous Gate", "ef_verglas_gate", Factions.NEUTRAL)
        ...
        ogTear.addTag(EFTags.VERGLAS_GATE)
        var mirrorTear = focus.mirroredEntity.starSystem.addCustomEntity("${focus.mirroredEntity.starSystem.id}_verglas_tear", "Anomalous Gate", "ef_verglas_gate", Factions.NEUTRAL)
        ...
        mirrorTear.addTag(EFTags.VERGLAS_GATE)
        /*
        if (Global.getSettings().allShipHullSpecs.find { it.hullId == "nebula" } != null)
        {
            ...
            nebulaWreck.addTag(EFTags.TAG_CHIRAL_NEBULA)
        }
        */

        if (ogPlugin is VerglasGate)
        {
            ogPlugin.teleportLocation = mirrorTear
        }

        if (mirrorPlugin is VerglasGate)
        {
    ...
    {
        //faction + station
        val fleet = FleetFactoryV3.createEmptyFleet("ef_verglas", FleetTypes.BATTLESTATION, null)
        val member: FleetMemberAPI = Global.getFactory().createFleetMember(FleetMemberType.SHIP, "ef_station_derelict_survey_mothership_Standard")
        ...
        val activeFleets = VestigeBaseFleetManager(fleet,
            3f,
            3,
            6,
            25f,
        ...
        //victus
        if (Global.getSettings().allShipHullSpecs.find { it.hullId == "ef_victus" } != null) {

            var derelictVictusParams = DerelictShipEntityPlugin.createVariant("ef_victus_Outdated", Random(), 0f)
            val nebulaWreck: CustomCampaignEntityAPI = fleet.starSystem.addCustomEntity(null, SalvageEntityGeneratorOld.getSalvageSpec(Entities.WRECK).getNameOverride()
                    , Entities.WRECK, Factions.NEUTRAL, derelictVictusParams)

            nebulaWreck.setCircularOrbit(fleet.orbitFocus, MathUtils.getRandomNumberInRange(0f, 360f), 900f, 250f)
            //nebulaWreck.addTag(RATTags.TAG_CHIRAL_NEBULA)

            /*
            DerelictShipEntityPlugin.DerelictShipData params = DerelictShipEntityPlugin.createVariant(variantId, random, DerelictShipEntityPlugin.getDefaultSModProb());
            if (params != null) {
